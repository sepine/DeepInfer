...
import logging
import json
from data_loader_param import load_datasets_and_vocabs
from model_gene import TypeEncoderDecoder
from model_classify import BasicTypeClassifier, NumInference
from trainer_param import trainer
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='classify',
                        choices=['classify', 'gene'],
                        help='which type of the model should we select')

    parser.add_argument('--module', type=str, default='param',
                        choices=['param', 'return'],
...
def main():
    ...
    train_dataset, test_dataset, opid2vec, \
        opcode2idx, idx2opcode, token2idx, idx2token, default_vocab_len = load_datasets_and_vocabs(args)
    end_time = datetime.now()
    logger.info('Load dataset spends %s' % str((end_time - start_time).seconds))
    args.default_vocab_len = default_vocab_len
    args.test_dataset_length = len(test_dataset)

    # Build Model
    model = None
    if args.pos == 'num':
        model = NumInference(opid2vec, token2idx, args)
    else:
        if args.model == 'gene':
            model = TypeEncoderDecoder(opid2vec, opcode2idx, token2idx, args)

        elif args.model == 'classify':
            model = BasicTypeClassifier(opid2vec, token2idx, args)
    ...
    if args.model == 'gene':
        ...
        indicators, internal_time = trainer(args, model, train_dataset, test_dataset, idx2token, token2idx, schedule, None)  # writer
        ...
        indicators, internal_time = trainer(args, model, train_dataset, test_dataset, idx2token, token2idx, None, None)
        logger.info('Model testing spends average %s s' % str(internal_time / args.test_dataset_length))
